...
export default function QuinielaDetalle() {
  ...
  const finalizada = quinielaDetalle.estado === 'FINALIZADA';
  ...
  return (
    <div className="quiniela-detalle">
      <header className="header">
        ...
        <span className={`estado-badge estado-${quinielaDetalle.estado.toLowerCase()}`}>
          {quinielaDetalle.estado}
        </span>
      </header>

      {finalizada && quinielaDetalle.ganadorNombre && (
        <div className="winner-banner">
          🏆 Ganador: <strong>{quinielaDetalle.ganadorNombre}</strong>
        </div>
      )}
            ...
            <table className="leaderboard">
              ...
              <tbody>
                {leaderboard.map((entry: LeaderboardEntry) => (
                  <tr key={entry.usuario.id} className={entry.usuario.nombre === quinielaDetalle.ganadorNombre ? 'winner-row' : ''}>
