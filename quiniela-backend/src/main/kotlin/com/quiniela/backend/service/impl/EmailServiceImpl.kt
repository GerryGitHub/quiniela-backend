...
class EmailServiceImpl(
    ...
    override fun sendPasswordResetEmail(email: String, nombre: String, token: String) {
    ...
    override fun sendVerificationEmail(email: String, nombre: String, token: String) {
        if (resendApiKey.isBlank()) {
            throw IllegalStateException("Servicio de correo no configurado")
        }
        ...
        val payload = mapOf(
            "from" to emailFrom,
            "to" to listOf(email),
            "subject" to "Código de verificación - QGol",
            "html" to htmlBody
        )
